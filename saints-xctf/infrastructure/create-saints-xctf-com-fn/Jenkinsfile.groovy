...
def checkoutRepos() {
    ...
    genericsteps.checkoutRepo('saints-xctf-functions', 'master')
}

def createLambdaZipFiles() {
    dir('saints-xctf-functions/forgot-password') {
        sh """
            yarn install --production=true
            zip -r SaintsXCTFForgotPasswordEmail.zip .
            cp SaintsXCTFForgotPasswordEmail.zip ../../saints-xctf-infrastructure/saints-xctf-com-fn/modules/email-lambda
        """
    }

    dir('saints-xctf-functions/upload-profile-picture') {
        sh """
            yarn install --production=true
            zip -r SaintsXCTFUassetUser.zip .
