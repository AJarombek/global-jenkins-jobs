/**
 * Jenkins script that uses Terraform to destroy a database deployment AWS Lambda function for a SaintsXCTF MySQL
...
pipeline {
    ...
    parameters {
        ...
        choice(
            ...
            description: 'Environment to destroy the database deployment function.'
        )
    }
    options {
        ansiColor('xterm')
...
def terraformInit() {
    INFRA_DIR = "repos/saints-xctf-infrastructure/database-deployment/env/$params.environment"
...
def postScript() {
    def bodyTitle = "Destroy saints-xctf-infrastructure $params.environment Database Deployment."
    def bodyContent = ""
